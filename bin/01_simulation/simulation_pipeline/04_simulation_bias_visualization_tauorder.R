...
for (tp in tail_porp){
        ...
        for (vs in valid_snps){
            ...
            for (ts in tau_scene){
                ...
                for (st in sim_times){
                    ...
                    late_all_mat <- as.data.frame(matrix(rnorm(10000 * 1, mean = tau_estimate$LATE_all, sd = 0), nrow=10000))
                    late_few_mat <- as.data.frame(matrix(rnorm(10000 * 1, mean = tau_estimate$LATE_few, sd = 0), nrow=10000))
                    ite_cf_mat <- as.data.frame(matrix(rnorm(10000 * 1, mean = tau_estimate$ITE_CF, sd = 0), nrow=10000))
                    ite_ivcf_all_mat <- as.data.frame(matrix(rnorm(10000 * 1, mean = tau_estimate$ITE_IVCF_all, sd = 0), nrow=10000))
    ...
    ggsave(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/plot/bias/all_bias_", tp * 100,".png"), p.final, dpi=300, 
