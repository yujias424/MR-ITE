...
for (tp in tail_porp){
        ...
        for (vs in valid_snps){
            ...
            for (ts in tau_scene){
                ...
                for (st in sim_times){
                    ...
                    tau_estimate <- fread(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/res/extra_analysis_for_paper_revision/01_simulation/tau/estimate/", 
                                                    ps, "_", vs, "_", ts, "_", st, "_estimate.csv.gz"))
                    tau_std <- fread(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/res/extra_analysis_for_paper_revision/01_simulation/tau/std/", 
            ...
            stat.test.ite.greater <- bias_df_res %>%
                group_by(tauscene) %>%
                t_test(Bias ~ Methods, paired = T, alternative = "greater",
                        comparisons = list(c("MR-ITE (keep pleiotropy)", "MR-ITE (remove pleiotropy)")))

            stat.test.ate <- bias_df_res %>%
                group_by(tauscene) %>%
                t_test(Bias ~ Methods, paired = T, alternative = "greater", # two.sided, greater, less
    ...
    ggsave(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/plot/extra_analysis_for_paper_revision/bias/all_bias_diffN_", tp * 100,".png"), p.final, dpi=300, 
