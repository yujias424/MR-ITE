...
split_half_testing <- function(X, Y, Z,
                               W,
                               binary = F,
                               col_names = NULL,
                               no_repeats = 10,
  ...
  #' 
  #' @param X: the X X matrix
  #' @param Y: outcome
  #' @param Z: instrument
  #' @param W: W assignment
  #' @param binary: binary W or not
  ...
  for (i in seq(no_repeats)) {
    ...
    if (binary) {
      treat <- seq(no.obs)[W == 1]
      control <- seq(no.obs)[W == 0]
    ...
    tau.forest.train <- cf.estimator(X = X[trainId, ], Y = Y[trainId], W = W[trainId], Z = Z[trainId], num.trees = num.trees, 
                                      node.size = min_node_size, ci.group.size = ci.group.size, seed = seed) # we would prefer a larger num.trees here, to guarantee a better CI as we don't want CI to be too conservative.
    tau.pred.train.a <- predict(tau.forest.train, estimate.variance = T, num.threads = n_core)
    tau.pred.test.a <- predict(tau.forest.train, newdata = X[-trainId, ], estimate.variance = T, num.threads = n_core)
    
    tau.forest.test <- cf.estimator(X = X[-trainId, ], Y = Y[-trainId], W = W[-trainId], Z = Z[-trainId], num.trees = num.trees, 
    ...
    tau.pred.test.b <- predict(tau.forest.test, newdata = X[trainId, ], estimate.variance = T, num.threads = n_core)
...
ivgrf <- function(X, Y, W, Z, Y.hat = NULL, W.hat = NULL, Z.hat = NULL, 
                  num.trees = 5000, node.size = 10, sample.fraction = 0.2, ci.group.size = 5, seed = NULL) {
...
assess.explained.tau.fixed.YW.risk <- function(fitted.obj, newdata = NULL, Y, Y.hat, W, W.hat, Z, Z.hat, constant.tau = NULL) {
  ...
  tau.pred <- predict(fitted.obj, newdata = newdata, estimate.variance = TRUE, num.threads = 8)
  tau.1 <- Y - Y.hat
  center.Z <- Z - Z.hat
...
paralleled.perm.cf <- function(X,
  ...
  #' @param reestimate: whether to re-estimate the Y.hat, Z.hat and W.hat using the permuted X.
  #' 
  #' @return: results containing permutation variance p-value and permutation risk variance.  
  
  col_names <- c("obs.Y", "obs.Y.hat", "obs.W", "obs.W.hat", "perm.Y.hat", "perm.W.hat")
  cf.estimator <- ivgrf # ifelse(is_tuned, cf.tuned, cf)
  ...
  perm.risk.mat <- foreach(i = seq(num.perm), .combine = "rbind", .options.multicore = list(set.seed = T)) %dopar%  {
    ...
    # What about we also permute for Z and Y? Follow the pemutation paper "Polygenic modelling of W effect heterogeneity"
    # Y <- Y[samp]
    # Y.hat <- Y.hat[samp]
    # Z <- Z[samp]
    # Z.hat <- Z.hat[samp]

    # I believe that we should use regression_forest() to re-restimate the Y.hat, W.hat and Z.hat use the permuted X
    if (reestimate == TRUE){
      Y.forest.perm <- regression_forest(sampled.X, Y, num.trees = num.trees, min.node.size = min.node.size)
      W.forest.perm <- regression_forest(sampled.X, W, num.trees = num.trees, min.node.size = min.node.size)
      Z.forest.perm <- regression_forest(sampled.X, Z, num.trees = num.trees, min.node.size = min.node.size)
...
permutate.covariates.testing <- function(X, 
  ...
  #' function is to permute X with Y, W, Y.hat, and W.hat, to investigate whether X contribute to HTE
...
adaptive.permutate.covariates.testing <- function(X, 
  ...
  #' @param W: W assignment
  #' @param W.hat: estimated W, E[W|X]
  #' @param W.hat.XZ: estimated W, E[W|X,Z]
  #' @param Z: instrument
  #' @param Z.hat: estimated Z, E[Z|X]
  #' @param fixed.YW.tau.risk: tau risk calculated in the default model
  #' @param tau.var: tau variance calculated in the default model
...
test_heterogeneity <- function(X, Y, W, Z, 
  ...
  iv.forest.continuous <- instrumental_forest(X, Y, W, Z, 
                                              num.threads = 30, num.trees = num.trees, 
