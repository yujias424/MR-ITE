...
split_half_testing <- function(covariates, Y, Z,
  ...
  #' @param num_trees: number of trees you want to use in the ivgrf
  ...
  for (i in seq(no_repeats)) {
    ...
    tau.forest.train <- cf.estimator(X = covariates[trainId, ], Y = Y[trainId], W = treatment[trainId], Z = Z[trainId], num.trees = num_trees, 
    ...
    tau.forest.test <- cf.estimator(X = covariates[-trainId, ], Y = Y[-trainId], W = treatment[-trainId], Z = Z[-trainId], num.trees = num_trees, 
...
paralleled.perm.cf <- function(covariates,
  ...
  #' @param sample_fraction: sample fraction used to estimate each tree. default: 0.5
  #' @param ci.group.size: ci.group.size setting for causal forest
  ...
  perm.risk.mat <- foreach(i = seq(mun_perm), .combine = "rbind", .options.multicore = list(set.seed = T)) %dopar%  {
    ...
    if (reestimate == TRUE){
      Y.forest.perm <- regression_forest(sampled.covariates, Y, num.trees = num_trees, min.node.size = min.node.size)
      W.forest.perm <- regression_forest(sampled.covariates, treatment, num.trees = num_trees, min.node.size = min.node.size)
      Z.forest.perm <- regression_forest(sampled.covariates, Z, num.trees = num_trees, min.node.size = min.node.size)
...
adaptive.permutate.covariates.testing <- function(X, 
  ...
  permute.var.pval <- mean(old.perm.risk.mat[, 1] >  tau.var) # not quite sure about these two 
