...
for (ml in models){
  for (t in 1:length(traits)){
    for (d in diseases){
      ...
      # binary W
      iv.tau.binaryW <- fread(paste0("/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/01_table/", traits[t], "/", d, "/01_individual_treatment_effect/ivgrf_full_binaryW_pvalue_", ml, ".csv"))
      cf.tau.binaryW <- fread(paste0("/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/01_table/", traits[t], "/", d, "/01_individual_treatment_effect/cf_full_binaryW_pvalue_", ml, ".csv"))
      driv.tau.binaryW <- fread(paste0("/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/01_table/", traits[t], "/", d, "/01_individual_treatment_effect/driv_full_binaryW_te_ul_", ml, ".csv"))

      # making plot (continuous W)
      iv.tau <- iv.tau.continuousW$tau
      iv.tau.sig <- iv.tau.continuousW$tau[iv.tau.continuousW$tau.p.adjust<0.1]
      ...
      driv.tau.sig <- driv.tau.continuousW$point[driv.tau.continuousW$p_value_corrected<0.1]
      ...
      cf.tau.sig <- cf.tau.continuousW$tau[cf.tau.continuousW$tau.p.adjust<0.1]
      ...
      taus.iv.plot.continuousW <- c(iv.tau, driv.tau, iv.tau.sig, driv.tau.sig) 
      taus.cf.plot.continuousW <- c(cf.tau, cf.tau.sig) 
      var.iv.plot.continuousW <- c(rep("IV-CF", length(iv.tau)), rep("DRIV", length(driv.tau)), rep("IV-CF", length(iv.tau.sig)), rep("DRIV", length(driv.tau.sig))) 
      var.cf.plot.continuousW <- c(rep("CF", length(cf.tau)), rep("CF", length(cf.tau.sig))) 
      sig.iv.plot.continuousW <- c(rep("Origin", length(iv.tau)), rep("Origin", length(driv.tau)), rep("P.adj < 0.1", length(iv.tau.sig)), rep("P.adj < 0.1", length(driv.tau.sig))) 
      sig.cf.plot.continuousW <- c(rep("Origin", length(cf.tau)), rep("P.adj < 0.1", length(cf.tau.sig))) 
      ...
      plot.taus.iv.continuousW$significance <- factor(plot.taus.iv.continuousW$significance, levels = c("Origin", "P.adj < 0.1"))
      ...
      plot.taus.cf.continuousW$significance <- factor(plot.taus.cf.continuousW$significance, levels = c("Origin", "P.adj < 0.1"))
      ...
      p.iv <- ggplot(plot.taus.iv.continuousW, aes(x=tau, fill=significance)) +
        ...
        scale_fill_manual(name=TeX("$\\tau$ type"), values=c("Origin"="lightblue", "P.adj < 0.1"="red"), labels=c("Origin"=TeX("Origin $\\tau$"), "P.adj < 0.1"=TeX("P.adj < 0.1 $\\tau$"))) +
      ...
      p.cf <- ggplot(plot.taus.cf.continuousW, aes(x=tau, fill=significance)) +
        ...
        theme(text = element_text(size = 35), plot.title = element_text(hjust = 0.5), 
              ...
              legend.position="right") + 
        geom_vline(xintercept=0, color="red", linetype="dashed") + 
        xlab(TeX("$\\tau$ (CATE)")) + ylab("Count") + # xlim(layer_scales(p.iv)$x$range$range[1], layer_scales(p.iv)$x$range$range[2])
        xlim(min(plot.taus.cf.continuousW$tau, plot.taus.iv.continuousW$tau), max(plot.taus.cf.continuousW$tau, plot.taus.iv.continuousW$tau))
      ...
      iv.tau.sig <- iv.tau.binaryW$tau[iv.tau.binaryW$tau.p.adjust<0.1]
      if (length(iv.tau.sig) == 0){
        iv.tau.sig <- c(mean(iv.tau))
      } else {
        iv.tau.sig <- iv.tau.sig
      ...
      driv.tau.sig <- driv.tau.binaryW$point[driv.tau.binaryW$p_value_corrected<0.1]
      if (length(driv.tau.sig) == 0){
        driv.tau.sig <- c(mean(driv.tau))
      } else {
        driv.tau.sig <- driv.tau.sig
      ...
      cf.tau.sig <- cf.tau.binaryW$tau[cf.tau.binaryW$tau.p.adjust<0.1]
      if (length(cf.tau.sig) == 0){
        cf.tau.sig <- c(mean(cf.tau))
      } else {
        cf.tau.sig <- cf.tau.sig
      ...
      taus.iv.plot.binaryW <- c(iv.tau, driv.tau, iv.tau.sig, driv.tau.sig) 
      taus.cf.plot.binaryW <- c(cf.tau, cf.tau.sig) 
      var.iv.plot.binaryW <- c(rep("IV-CF", length(iv.tau)), rep("DRIV", length(driv.tau)), rep("IV-CF", length(iv.tau.sig)), rep("DRIV", length(driv.tau.sig))) 
      var.cf.plot.binaryW <- c(rep("CF", length(cf.tau)), rep("CF", length(cf.tau.sig))) 
      sig.iv.plot.binaryW <- c(rep("Origin", length(iv.tau)), rep("Origin", length(driv.tau)), rep("P.adj < 0.1", length(iv.tau.sig)), rep("P.adj < 0.1", length(driv.tau.sig))) 
      sig.cf.plot.binaryW <- c(rep("Origin", length(cf.tau)), rep("P.adj < 0.1", length(cf.tau.sig))) 

      plot.taus.iv.binaryW <- data.frame("tau" = taus.iv.plot.binaryW, "variable" = var.iv.plot.binaryW, "significance" = sig.iv.plot.binaryW)
      plot.taus.iv.binaryW$significance <- factor(plot.taus.iv.binaryW$significance, levels = c("Origin", "P.adj < 0.1"))
      ...
      plot.taus.cf.binaryW$significance <- factor(plot.taus.cf.binaryW$significance, levels = c("Origin", "P.adj < 0.1"))
  ...
  ggsave(paste0("/home/yujia/Project/2023-07-20-individual_MR/res/03_plot/figure1_adjusted_pvalue_", ml, "_james.png"), histogram_p, dpi=300, 
