...
for (ml in models){
  for (t in 1:length(traits)){
    for (d in diseases){
      ...
      sig.iv.plot.continuousW <- c(rep("Origin", length(iv.tau)), rep("Origin", length(driv.tau)), rep("P.val < 0.1", length(iv.tau.sig)), rep("P.val < 0.1", length(driv.tau.sig))) 
      sig.cf.plot.continuousW <- c(rep("Origin", length(cf.tau)), rep("P.val < 0.1", length(cf.tau.sig))) 
      ...
      plot.taus.iv.continuousW$significance <- factor(plot.taus.iv.continuousW$significance, levels = c("Origin", "P.val < 0.1"))
      ...
      plot.taus.cf.continuousW$significance <- factor(plot.taus.cf.continuousW$significance, levels = c("Origin", "P.val < 0.1"))
      ...
      p.iv <- ggplot(plot.taus.iv.continuousW, aes(x=tau, fill=significance)) +
        ...
        scale_fill_manual(name=TeX("$\\tau$ type"), values=c("Origin"="lightblue", "P.val < 0.1"="red"), labels=c("Origin"=TeX("Origin $\\tau$"), "P.val < 0.1"=TeX("P.val < 0.1 $\\tau$"))) +
      ...
      sig.iv.plot.binaryW <- c(rep("Origin", length(iv.tau)), rep("Origin", length(driv.tau)), rep("P.val < 0.1", length(iv.tau.sig)), rep("P.val < 0.1", length(driv.tau.sig))) 
      sig.cf.plot.binaryW <- c(rep("Origin", length(cf.tau)), rep("P.val < 0.1", length(cf.tau.sig))) 

      plot.taus.iv.binaryW <- data.frame("tau" = taus.iv.plot.binaryW, "variable" = var.iv.plot.binaryW, "significance" = sig.iv.plot.binaryW)
      plot.taus.iv.binaryW$significance <- factor(plot.taus.iv.binaryW$significance, levels = c("Origin", "P.val < 0.1"))
      ...
      plot.taus.cf.binaryW$significance <- factor(plot.taus.cf.binaryW$significance, levels = c("Origin", "P.val < 0.1"))
      ...
      p.iv <- ggplot(plot.taus.iv.binaryW, aes(x=tau, fill=significance)) +
        geom_histogram(color="black",  bins = 50, position = 'identity', alpha=0.6) + 
        scale_fill_manual(name=TeX("$\\tau$ type"), values=c("Origin"="lightblue", "P.val < 0.1"="red"), labels=c("Origin"=TeX("$\\tau$"), "P.val < 0.1"=TeX("$\\tau$ with P.val < 0.1"))) +
        facet_grid(rows = vars(variable), scales = "free", labeller = label_parsed) + ggtitle(traits_name[t]) + 
        theme_classic() + 
        theme(text = element_text(size = 35), plot.title = element_text(hjust = 0.5), 
              axis.text = element_text(size = 35), axis.line=element_line(color='black'),
  ...
  ggsave(paste0("/home/yujia/Project/2023-07-20-individual_MR/res/03_plot/figure1_normal_pvalue_", ml, "_james.png"), histogram_p, dpi=300, 
