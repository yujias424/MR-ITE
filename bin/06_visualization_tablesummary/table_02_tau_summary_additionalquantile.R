...
matrix.ivcf <- matrix(nrow = 16, ncol = 26) # previous 16 since we have IST.
matrix.cf <- matrix(nrow = 16, ncol = 26)
matrix.driv <- matrix(nrow = 16, ncol = 26)
n <- 1
...
for (t in 1:length(traits)){
  for (d in diseases){
    for (md in models){
      ...
      matrix.ivcf[n*2-1, 17] <- "Continuous W"
      matrix.ivcf[n*2-1, 18] <- traits_name[t]
      matrix.ivcf[n*2-1, 19] <- d
      matrix.ivcf[n*2-1, 20] <- length(iv.tau.continuousW$tau.p.adjust)
      matrix.ivcf[n*2-1, 21] <- md
      matrix.ivcf[n*2-1, 22] <- length(which(iv.tau.continuousW$tau.p.adjust < 0.05))/length(iv.tau.continuousW$tau.p.adjust)
      matrix.ivcf[n*2-1, 23] <- length(which(iv.tau.continuousW$tau.pval < 0.05))/length(iv.tau.continuousW$tau.pval)
      ...
      matrix.ivcf[n*2-1, 26] <- abs((as.numeric(matrix.ivcf[n*2-1, 6]) - as.numeric(matrix.ivcf[n*2-1, 4]))/as.numeric(matrix.ivcf[n*2-1, 4]))
      ...
      matrix.cf[n*2-1, 17] <- "Continuous W"
      matrix.cf[n*2-1, 18] <- traits_name[t]
      matrix.cf[n*2-1, 19] <- d
      matrix.cf[n*2-1, 20] <- length(cf.tau.continuousW$tau.p.adjust)
      matrix.cf[n*2-1, 21] <- md
      matrix.cf[n*2-1, 22] <- length(which(cf.tau.continuousW$tau.p.adjust < 0.05))/length(cf.tau.continuousW$tau.p.adjust)
      matrix.cf[n*2-1, 23] <- length(which(cf.tau.continuousW$tau.pval < 0.05))/length(cf.tau.continuousW$tau.pval)
      ...
      matrix.cf[n*2-1, 26] <- abs((as.numeric(matrix.cf[n*2-1, 6]) - as.numeric(matrix.cf[n*2-1, 4]))/as.numeric(matrix.cf[n*2-1, 4]))
      ...
      matrix.driv[n*2-1, 17] <- "Continuous W"
      matrix.driv[n*2-1, 18] <- traits_name[t]
      matrix.driv[n*2-1, 19] <- d
      matrix.driv[n*2-1, 20] <- length(driv.tau.continuousW$p_value_corrected)
      matrix.driv[n*2-1, 21] <- md
      matrix.driv[n*2-1, 22] <- length(which(driv.tau.continuousW$p_value_corrected < 0.05))/length(driv.tau.continuousW$p_value_corrected)
      matrix.driv[n*2-1, 23] <- length(which(driv.tau.continuousW$p_value < 0.05))/length(driv.tau.continuousW$p_value)
      ...
      matrix.driv[n*2-1, 26] <- abs((as.numeric(matrix.driv[n*2-1, 6]) - as.numeric(matrix.driv[n*2-1, 4]))/as.numeric(matrix.driv[n*2-1, 4]))
      ...
      matrix.ivcf[n*2, 17] <- "Binary W"
      matrix.ivcf[n*2, 18] <- traits_name[t]
      matrix.ivcf[n*2, 19] <- d
      matrix.ivcf[n*2, 20] <- length(iv.tau.binaryW$tau.p.adjust)
      matrix.ivcf[n*2, 21] <- md
      matrix.ivcf[n*2, 22] <- length(which(iv.tau.binaryW$tau.p.adjust < 0.05))/length(iv.tau.binaryW$tau.p.adjust)
      matrix.ivcf[n*2, 23] <- length(which(iv.tau.binaryW$tau.pval < 0.05))/length(iv.tau.binaryW$tau.pval)
      ...
      matrix.ivcf[n*2, 26] <- abs((as.numeric(matrix.ivcf[n*2, 6]) - as.numeric(matrix.ivcf[n*2, 4]))/as.numeric(matrix.ivcf[n*2, 4]))
      ...
      matrix.cf[n*2, 17] <- "Binary W"
      matrix.cf[n*2, 18] <- traits_name[t]
      matrix.cf[n*2, 19] <- d
      matrix.cf[n*2, 20] <- length(cf.tau.binaryW$tau.p.adjust)
      matrix.cf[n*2, 21] <- md
      matrix.cf[n*2, 22] <- length(which(cf.tau.binaryW$tau.p.adjust < 0.05))/length(cf.tau.binaryW$tau.p.adjust)
      matrix.cf[n*2, 23] <- length(which(cf.tau.binaryW$tau.pval < 0.05))/length(cf.tau.binaryW$tau.pval)
      ...
      matrix.cf[n*2, 26] <- abs((as.numeric(matrix.cf[n*2, 6]) - as.numeric(matrix.cf[n*2, 4]))/as.numeric(matrix.cf[n*2, 4]))
      ...
      matrix.driv[n*2, 17] <- "Binary W"
      matrix.driv[n*2, 18] <- traits_name[t]
      matrix.driv[n*2, 19] <- d
      matrix.driv[n*2, 20] <- length(driv.tau.binaryW$p_value_corrected)
      matrix.driv[n*2, 21] <- md
      matrix.driv[n*2, 22] <- length(which(driv.tau.binaryW$p_value_corrected < 0.05))/length(driv.tau.binaryW$p_value_corrected)
      matrix.driv[n*2, 23] <- length(which(driv.tau.binaryW$p_value < 0.05))/length(driv.tau.binaryW$p_value)
      ...
      matrix.driv[n*2, 26] <- abs((as.numeric(matrix.driv[n*2, 6]) - as.numeric(matrix.driv[n*2, 4]))/as.numeric(matrix.driv[n*2, 4]))
...
fwrite(matrix.ivcf, "/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/05_summary_statistics/02_taus/method_IVCF_taus_additionquantile_2.csv")
fwrite(matrix.cf, "/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/05_summary_statistics/02_taus/method_CF_taus_additionquantile_2.csv")
fwrite(matrix.driv, "/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/05_summary_statistics/02_taus/method_DRIV_taus_additionquantile_2.csv")

matrix_final <- rbind(matrix.cf, matrix.driv, matrix.ivcf)
fwrite(matrix_final, "/home/yujia/Project/2023-07-20-individual_MR/res/02_ITE_analysis/05_summary_statistics/02_taus/method_taus_additionquantile_2.csv")
