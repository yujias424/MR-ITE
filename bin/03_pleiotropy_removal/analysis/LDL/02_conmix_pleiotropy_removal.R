...
for (ds in diseases){
  for (p in p.value.cutoff){
    ...
    ssdat <- fread(paste0("~/Project/2023-07-20-individual_MR/dat/05_two_sample_mr/", ds, "/LDL/merged_", ds, "_", p, ".txt.gz"))
    ...
    write.csv(selected_snps, file = paste0("~/Project/2023-07-20-individual_MR/dat/06_PRS_calculation/LDL/", ds, "/selected_snps_", p, ".csv"), 
              row.names = F, quote = F)
