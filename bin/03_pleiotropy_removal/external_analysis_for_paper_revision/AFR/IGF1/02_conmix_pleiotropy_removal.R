...
for (ds in diseases){
  for (p in p.value.cutoff){
    ...
    ssdat <- ssdat[ssdat$pval.exposure < 0.99,]
    ...
    write.csv(selected_snps, file = paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/06_PRS_calculation/external_analysis_for_paper_revision/IGF1/", ds, "/selected_snps_", p, ".csv"), 
              row.names = F, quote = F)
    # break
  }
