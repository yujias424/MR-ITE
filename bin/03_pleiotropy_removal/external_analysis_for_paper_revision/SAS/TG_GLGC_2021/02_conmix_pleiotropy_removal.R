...
for (ds in diseases){
  for (p in p.value.cutoff){
    ...
    # merge file is obtained from harmonize_TG.R file.
    ssdat <- fread(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/05_two_sample_mr/", ds, "/TG_GLGC_2021/merged_", ds, "_", p, ".txt.gz"))
    ...
    write.csv(selected_snps, file = paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/06_PRS_calculation/TG_GLGC_2021/", ds, "/selected_snps_", p, ".csv"), 
              row.names = F, quote = F)
