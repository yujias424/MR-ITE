...
for (ds in diseases){
  for (p in p.value.cutoff){
    ...
    ssdat <- fread(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/05_two_sample_mr/external_analysis_for_paper_revision/", ds, "/TC/merged_", ds, "_", p, ".SAS.txt.gz"))
    ...
    dir.create(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/06_PRS_calculation/external_analysis_for_paper_revision/PRSice2/SAS/TC/", ds, "/"), recursive = T, showWarnings = F)

    write.csv(selected_snps, file = paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/06_PRS_calculation/external_analysis_for_paper_revision/PRSice2/SAS/TC/", ds, "/selected_snps_", p, ".SAS.csv"), 
              row.names = F, quote = F)
