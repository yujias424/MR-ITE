...
for (ds in diseases){
  for (p in p.value.cutoff){
    ...
    # merge file is obtained from harmonize_CRP.R file.
    ssdat <- fread(paste0("~/Project/2023-07-20-individual_MR/dat/05_two_sample_mr/", ds, "/CRP/merged_", ds, "_", p, ".txt.gz"))
    ...
    write.csv(selected_snps, file = paste0("~/Project/2023-07-20-individual_MR/dat/06_PRS_calculation/CRP/", ds, "/selected_snps_", p, ".csv"), 
              row.names = F, quote = F)
    # break
  }
