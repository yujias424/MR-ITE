...
diseases <- c("CAD")
p.value.cutoff <- c(0.1, 0.2)

for (ds in diseases){
  for (p in p.value.cutoff){
    
    # merge file is obtained from harmonize_IGF1.R file.
    ssdat <- fread(paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/05_two_sample_mr/external_analysis_for_paper_revision/", ds, "/IGF1/merged_", ds, "_", p, ".txt.gz"))
    ssdat <- as.data.frame(ssdat)

    ssdat <- ssdat[ssdat$pval.exposure < p,]
    ...
    write.csv(selected_snps, file = paste0("/mnt/md0/yujia/project/2023-07-20-individual_MR/dat/06_PRS_calculation/external_analysis_for_paper_revision/LDPred2/EUR/IGF1/", ds, "/selected_snps_", p, ".csv"), 
              row.names = F, quote = F)
    # break
  }
