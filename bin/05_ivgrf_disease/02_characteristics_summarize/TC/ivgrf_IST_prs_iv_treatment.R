...
#' causal relationship between Total Cholestrol and ischemic stroke.
#' 
#' @author: Yujia Shi
#' @date: 2022.09.01
...
X <- fread("~/Project/2022-09-01-individual_MR/dat/analysis/pheno_data/TC/ukbb.covariate.TC.complete", sep = "\t")
Z <- fread("~/Project/2022-09-01-individual_MR/dat/analysis/prs/mgdL/score_std/TC/ischemic_stroke/TC_prs.best", sep = " ")
W <- fread("~/Project/2022-09-01-individual_MR/dat/analysis/pheno_data/TC/ukbb.phenotype.TC.complete.mgdL", sep = "\t")
...
samples <- fread("~/Project/2022-09-01-individual_MR/dat/analysis/psm_res/TC/ischemic_stroke_psm_3.csv")
samples <- samples$sample

W <- W[W$IID %in% samples, c("30690-0.0")]
X <- X[X$IID %in% samples, 3:dim(X)[2]]
Y <- Y[Y$IID %in% samples, 2]
Z <- Z[Z$IID %in% samples, 4]

W.vector <- as.vector(W$`30690-0.0`)
...
W.vector.binary.cutoff <- as.integer(ifelse(W.vector <= 220, 1, 0))
...
write.csv(continuous_summary, file = "~/Project/2022-09-01-individual_MR/res/table/summary_statistics/TC_IS_continuous.csv")
write.csv(binary_summary, file = "~/Project/2022-09-01-individual_MR/res/table/summary_statistics/TC_IS_binary.csv")
