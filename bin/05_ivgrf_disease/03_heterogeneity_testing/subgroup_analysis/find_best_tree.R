#' This is the code to find the best tree from the forest, we will use this tree to subgroup patient in training set
...
#' @param Z.orig Z values that indicate instruments; used for instrumental forest.
#' @param Z.hat Fitted Z values for instrumental forest
#' @param M.hat M.hat for causal forest
#' @param Tau.hat Fitten tau values for causal forest
#'
...
find_leaf <- function(x, tree, prune_info) {
  ...
  while(!prune_info[[idx]]$is_pruned_leaf) {
    # print(n$split_variable)
    # print(x)
    # print(x[1, n$split_variable])
    # print(n$split_value)
    # print("  ")
    if (x[1, n$split_variable] <= n$split_value) {
      idx <- n$left_child
    } else {
      idx <- n$right_child
    }
...
#' @return The leaf that the data point belongs to.
#' @export
find_leaf_notpruned <- function(x, tree) {
  x <- as.matrix(x)
  nodes <- tree$nodes
...
#' Function to assign subgroup index to patients
#' 
#' @param tree the best tree object
#' @param X covariates set
#' 
#' @return  the patients subgroup index.
subgroup_patients <- function(tree, X){
  
  # tmp.X <- X[-tree$drawn_samples, ]
  tmp.X <- X
  subgroup.index <- c()
