...
suppressPackageStartupMessages({
  ...
  source("/mnt/md0/yujia/project/2023-07-20-individual_MR/bin/05_ivgrf_disease/03_heterogeneity_testing/subgroup_analysi_updated/find_best_tree.R")
})

# IGF1 - CAD Binary trait
...
Y.rf.model <- regression_forest(X.model3.mat.PC, Y.model3.vector,
                                ...
                                sample.fraction = 0.03, min.node.size = 250, ci.group.size = 100)
...
# subgroup.index.train <- subgroup_patients(best.tree, X.model3.mat.noPC[best.tree$drawn_samples])
# table(subgroup.index.train)/2
...
length(subgroup.index.train[subgroup.index.train == 3])
length(subgroup.index.train[subgroup.index.train == 3])
...
p.validation <- ggplot(sample.df, aes(factor(class), y=mean, ymin=mean - 1.96 * std.error, ymax = mean + 1.96 * std.error)) +
  ...
  theme_classic() + xlab("Subgroup") + ylab("Local Average Treatment Effect") + ggtitle("IGF-1\nContinuous Treatment") + 
...
ggsave("/mnt/md0/yujia/project/2023-07-20-individual_MR/res/03_plot/supp_fig1_IGF1_CAD_continuous.png", plot = p.validation, dpi = 300,
       height = 10, width = 10, scale = 1)

# save.image("/mnt/md0/yujia/project/2023-07-20-individual_MR/res/02_ITE_analysis/03_validation_update/continuousW_IGF1_CAD.RData")
# ========================================================================================
...
library(rsvg)
cat(DiagrammeRsvg::export_svg(tree_plot), file = '/mnt/md0/yujia/project/2023-07-20-individual_MR/res/03_plot/update_besttree/continuousW_IGF1_CAD_besttree.svg')
tree_plot %>% export_svg %>% charToRaw %>% rsvg_pdf("'/mnt/md0/yujia/project/2023-07-20-individual_MR/res/03_plot/update_besttree/continuousW_IGF1_CAD_besttree.pdf")

# ANOVA
# https://statpages.info/anova1sm.html ANOVA res: 0.0488

#  	SS	DoF	MS	F	P
# Between	6.2e+00	5	1.247	2.2268	0.0488
# Within	1.5e+05	267766	0.56		
# Total	1.5e+05	267771	