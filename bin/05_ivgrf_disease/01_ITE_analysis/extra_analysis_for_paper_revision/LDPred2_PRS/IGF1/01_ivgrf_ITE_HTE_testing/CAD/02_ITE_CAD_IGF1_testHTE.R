...
X.model3.mat  <- X.model3.mat  %>% 
                  rename("Gender"="22001-0.0", "Age"="21022-0.0", "diastolic blood pressure"="4079-0.0", "systolic blood pressure"="4080-0.0", "Townsend deprivation index"="189-0.0",   
                         ...
                         "Blood pressure medication"="Blood_pressure_medication", "Cholesterol lowering medication"="Cholesterol_lowering_medication", "Insulin"="Insulin", # "No medication"="No_medication", 
                          ...
                          "Urea"="30670-0.0", "Phosphate"="30810-0.0", "SHBG"="30830-0.0", "Testosterone"="30850-0.0", 
...
perm.pvalue.continuousW.model1b <- test_heterogeneity(X.model1b.mat, Y.model1b.vector, W.model1b.vector, Z.model1b.vector,
                                                      num.trees = 3000, seed = 309,
                                                      sample.fraction = 0.03, min.node.size = 300, ci.group.size = 2, num.strap = 100, min.perm = 50, num.threads = 40,
                                                      constant.tau = res.model1b$coefficients[["W"]])
message("Model 1b")
print(perm.pvalue.continuousW.model1b)

perm.pvalue.continuousW.model3 <- test_heterogeneity(X.model3.mat, Y.model3.vector, W.model3.vector, Z.model3.vector,
                                                      num.trees = 3000, seed = 309,
                                                      sample.fraction = 0.03, min.node.size = 200, ci.group.size = 2, num.strap = 100, min.perm = 50, num.threads = 40,
                                                      constant.tau = res.model3$coefficients[["W"]])
message("Model 3")
...
#                                                      sample.fraction = 0.03, min.node.size = 200, ci.group.size = 2, num.strap = 100, num.threads = 40, min.perm = 50)
# message("Model 3")
# print(perm.pvalue.continuousW.model3) 

permute.test.dat.continuousW <- data.frame("Perm_Var" = c(perm.pvalue.continuousW.model1b[1], perm.pvalue.continuousW.model3[1]),
                                           "Perm_Risk" = c(perm.pvalue.continuousW.model1b[2], perm.pvalue.continuousW.model3[2]))
row.names(permute.test.dat.continuousW) <- c("Model 1", "Model 3")
write.csv(permute.test.dat.continuousW, file = "/mnt/md0/yujia/project/2023-07-20-individual_MR/res/02_ITE_analysis/01_table/IGF1/CAD/02_hte_testing/permutationtest_IGF1_CAD_continuousW.csv")
...
# permute.test.dat.binaryW <- data.frame("Perm_Var" = c(perm.pvalue.binaryW.model1b[1], perm.pvalue.binaryW.model3[1]),
#                                        "Perm_Risk" = c(perm.pvalue.binaryW.model1b[2], perm.pvalue.binaryW.model3[2]))
# row.names(permute.test.dat.binaryW) <- c("Model 1", "Model 3")
# write.csv(permute.test.dat.binaryW, file = "/mnt/md0/yujia/project/2023-07-20-individual_MR/res/02_ITE_analysis/01_table/IGF1/CAD/02_hte_testing/permutationtest_IGF1_CAD_binaryW.csv")
